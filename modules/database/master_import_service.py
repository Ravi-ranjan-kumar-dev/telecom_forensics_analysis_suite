...
def _successful_fingerprint_exists(
    connection: duckdb.DuckDBPyConnection,
    fingerprint: str,
) -> bool:
...
def _fast_file_fingerprint(
    path: Path,
) -> str:
    """
    Build a fast duplicate-detection fingerprint.

    The fingerprint contains file metadata plus the first and last
    1 MiB. It avoids a second full read of very large SDR files.
    """
...
def _find_header_companion(
    path: Path,
) -> tuple[list[str], str, Path] | None:
    """
    Find a separate header file only for a headerless export.

    Normal CSV files keep their own embedded header. Generic companion
    headers are considered only for files whose name ends with `_data`.
    """
...
def _read_text_preview(
    ...
    companion = _find_header_companion(
        path
    )

    if companion is not None:
        columns, delimiter, companion_path = (
            companion
        )
    ...
    header_line = _detect_header_line(
        path
    )
...
def detect_master_data_type(
    ...
    if suffix not in SUPPORTED_MASTER_SUFFIXES:
        raise ValueError(
            "Unsupported master data file type: "
    ...
    if suffix in TEXT_MASTER_SUFFIXES:
        preview, metadata = _read_text_preview(
            path
        )
    else:
        preview = _read_input_file(
            path
        )
        metadata = {
            "header_line": None,
    ...
    if sdr_score <= 0 and cgi_score <= 0:
        raise ValueError(
            "Could not detect SDR or CGI master-data columns. "
            f"Columns found: {list(preview.columns)}"
        )
    ...
    if sdr_score == cgi_score:
        if strong_sdr and not strong_cgi:
            ...
            raise ValueError(
                "The file contains ambiguous SDR and CGI columns. "
                "Use a clean source file with one master-data type."
            )
    else:
        import_type = (
            "SDR"
...
def _create_database_backup(
    *,
    ...
    if not database_path.is_file():
        raise FileNotFoundError(
            f"Master database not found: {database_path}"
        )
    ...
    if free_bytes < required_bytes:
        raise RuntimeError(
            "Insufficient free space for the automatic database "
            "backup. Required approximately "
            f"{required_bytes / 1024 / 1024 / 1024:.2f} GiB; "
...
def _write_json_log(
    ...
    temporary_path = log_path.with_suffix(
        ".json.tmp"
    )
    ...
    return f"""
        CASE
            WHEN length({digits}) = 12
             AND starts_with({digits}, '91')
                THEN right({digits}, 10)
            WHEN length({digits}) = 11
             AND starts_with({digits}, '0')
                THEN right({digits}, 10)
...
def _text_relation_sql(
    ...
    delimiter_literal = _sql_literal(
        delimiter
    )

    header_columns = detection.get(
        "header_columns",
        [],
    )
...
def _create_sdr_text_stage(
    ...
    relation_sql = _text_relation_sql(
        path,
        detection,
    )
    ...
    if mobile_column is None:
        raise ValueError(
            "Mobile/MSISDN column was not found in the SDR file. "
            f"Columns found: {columns}"
        )

    expressions: dict[str, str] = {
        "mobile_number": _mobile_expression(
            mobile_column
        )
    }
    ...
    for target_column in (
        column
        for column in SDR_TARGET_COLUMNS
        if column not in {
            "mobile_number",
            "source_file",
        }
    ):
        source_column = _match_column(
            columns,
            SDR_COLUMN_ALIASES[
                target_column
            ],
    ...
    source_file_literal = _sql_literal(
        path.name
    )

    connection.execute(
        "DROP TABLE IF EXISTS incoming_sdr_all"
    )
    connection.execute(
        "DROP TABLE IF EXISTS incoming_sdr_valid"
    )
    ...
    connection.execute(
        ...
        SELECT
            {expressions["mobile_number"]}
                AS mobile_number,
            {expressions["subscriber_name"]}
                AS subscriber_name,
            {expressions["father_name"]}
                AS father_name,
            {expressions["address"]}
                AS address,
            {expressions["id_type"]}
                AS id_type,
            {expressions["id_number"]}
                AS id_number,
            {expressions["operator"]}
                AS operator,
            {expressions["circle"]}
                AS circle,
            {expressions["activation_date"]}
                AS activation_date,
            {expressions["caf_number"]}
                AS caf_number,
            {source_file_literal}
                AS source_file,
            current_timestamp
...
def _create_sdr_excel_stage(
    connection: duckdb.DuckDBPyConnection,
    path: Path,
) -> dict[str, int]:
    """Prepare a normal-sized Excel SDR file."""

    raw = _read_input_file(
        path
    )
    ...
    if mobile_column is None:
        raise ValueError(
            "Mobile/MSISDN column was not found."
        )

    normalized = raw[
        mobile_column
    ].map(
        _normalize_mobile_python
    )
...
def _null_safe_difference(
    ...
    for column in columns:
        ...
        comparisons.append(
            "("
            f"CAST({left_alias}.{quoted} AS VARCHAR) "
            "IS DISTINCT FROM "
            f"CAST({right_alias}.{quoted} AS VARCHAR)"
            ")"
        )
...
def _create_existing_sdr_stage(
    connection: duckdb.DuckDBPyConnection,
) -> int:
    """Resolve current SDR records with primary-table priority."""

    connection.execute(
        "DROP TABLE IF EXISTS existing_sdr_best"
    )
    ...
    if _table_exists(
        connection,
        "sdr_subscribers_large",
    ):
        base_rows = _table_count(
            connection,
            "sdr_subscribers_large",
        )
        ...
        connection.execute(
            ...
            FROM (
                ...
                FROM sdr_subscribers_large
                    AS large_data
                INNER JOIN incoming_sdr_valid
                    AS incoming
                    ON incoming.mobile_number
                     = large_data.mobile_number
                LEFT JOIN sdr_subscribers
                    AS primary_data
                    ON primary_data.mobile_number
                     = large_data.mobile_number
                WHERE primary_data.mobile_number
                      IS NULL
            )
            WHERE ranking = 1
            """
...
def _upsert_sdr_stage(
    ...
    base_rows = _create_existing_sdr_stage(
        connection
    )

    difference = _null_safe_difference(
        "incoming",
        "existing",
        SDR_COMPARE_COLUMNS,
    )
    ...
    connection.execute(
        "DROP TABLE IF EXISTS incoming_sdr_changes"
    )
    ...
    inserted_rows = int(
        connection.execute(
            ...
            WHERE existing_mobile_number IS NULL
            """
        ).fetchone()[0]
    )
    ...
    updated_rows = int(
        connection.execute(
            ...
            SELECT COUNT(*)
            FROM incoming_sdr_changes
            WHERE existing_mobile_number IS NOT NULL
            """
        ).fetchone()[0]
    )
...
def _prepare_cgi_dataframe(
    path: Path,
) -> tuple[pd.DataFrame, dict[str, int]]:
    """Read and deduplicate one CGI master workbook or file."""

    prepared_frames = read_cgi_master_file(
        path
    )
    ...
    if not usable_frames:
        raise ValueError(
            "No valid CGI rows were found in the selected file."
        )
...
def _upsert_cgi_dataframe(
    ...
    connection.register(
        "incoming_cgi_dataframe",
        dataframe,
    )

    connection.execute(
        "DROP TABLE IF EXISTS incoming_cgi_valid"
    )
    ...
    connection.execute(
        "DROP TABLE IF EXISTS incoming_cgi_changes"
    )
    ...
    inserted_rows = int(
        connection.execute(
            """
            SELECT COUNT(*)
            FROM incoming_cgi_changes
            WHERE existing_cgi IS NULL
            """
        ).fetchone()[0]
    )
    ...
    updated_rows = int(
        connection.execute(
            ...
            SELECT COUNT(*)
            FROM incoming_cgi_changes
            WHERE existing_cgi IS NOT NULL
            """
        ).fetchone()[0]
    )
...
def import_master_data_file(
    ...
    *,
    ...
    try:
        ...
        result.source_size_bytes = int(
            stat.st_size
        )
        result.source_mtime_ns = int(
            stat.st_mtime_ns
        )
        result.fingerprint = (
            _fast_file_fingerprint(
                path
            )
        )

        detection = detect_master_data_type(
            path
        )

        result.import_type = str(
            detection[
                "import_type"
            ]
        )
        ...
        with duckdb.connect(
            ...
            if _successful_fingerprint_exists(
                connection,
                result.fingerprint,
            ):
                result.status = (
                ...
                result.message = (
                    "The same master-data file was already "
            ...
            if (
                result.import_type == "SDR"
                and result.source_size_bytes
                >= 1024 * 1024 * 1024
                and _large_sdr_source_exists(
                    connection,
                    path.name,
                )
            ):
                result.status = (
                    "SKIPPED_EXISTING_BASE"
                )
                result.base_rows = _table_count(
                    connection,
                    "sdr_subscribers_large",
                )
                result.message = (
                    "This large SDR source file is already "
                    "present in the historical SDR base table."
                )
                result.duration_seconds = round(
                    time.perf_counter()
                    - started_clock,
        ...
        with duckdb.connect(
            ...
            if result.import_type == "SDR":
                if (
                    path.suffix.lower()
                    in TEXT_MASTER_SUFFIXES
                ):
                    preparation = (
                        _create_sdr_text_stage(
                            connection,
                            path,
                            detection,
                        )
                    ...
                    preparation = (
                        _create_sdr_excel_stage(
                            connection,
                            path,
                        )
                    )

                changes = _upsert_sdr_stage(
                    connection
                )
            ...
            result.message = (
                "Master data imported and verified successfully."
            )
            result.duration_seconds = round(
                time.perf_counter()
                - started_clock,
