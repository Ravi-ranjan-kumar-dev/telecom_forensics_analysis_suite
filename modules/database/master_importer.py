...
SUPPORTED_IMPORT_SUFFIXES = {".csv", ".xlsx", ".xls", ".txt", ".tsv"}
...
def _has_known_master_columns(dataframe: pd.DataFrame) -> bool:
    ...
    This prevents first data row from being accepted as header.
    """
    known_columns = {
        "mobile_number",
        "mobile",
...
def _prepare_cgi_dataframe(raw: pd.DataFrame, source_file: str) -> pd.DataFrame:
    cgi_col = _first_existing_column(
        raw,
        [
            "cgi",
            "cell_id",
    ...
    mapping = {
        ...
        "police_station": ["police_station", "ps", "thana"],
        "address": ["address", "site_address", "tower_address", "location", "site_name"],
        "latitude": ["latitude", "lat"],
        "longitude": ["longitude", "long", "lng"],
    }

    output = pd.DataFrame()
    output["cgi"] = raw[cgi_col].map(normalize_cgi)
    ...
    output = output[output["cgi"].astype(str).str.strip().ne("")].copy()
...
def import_cgi_master_file(file_path) -> int:
    ...
    prepared = _prepare_cgi_dataframe(raw, path.name)
